...
def sort_by_start_date(file_name):
    ...
    lines = [
        "/".join(
            [
                (
                    datetime.strptime(item[0], "%Y-%m-%d/%H:%M").strftime(
                        "%Y-%m-%d/%H:%M"
                    )
                    if "/" in item[0]
                    else item
...
class app:
    def __init__(self, year, month):
        ...
        # 显示月份
        self.month_minus_button = tk.Button(
            self.root, text="<", command=self.month_minus
        )
        ...
        self.add_schedule_button = tk.Button(
            self.root, text="添加日程", command=self.add_schedule, font=("Arial", 18)
        )
        ...
        # 创建一个按钮，点击后显示所有日程
        self.show_all_schedule_button = tk.Button(
            self.frame, text="查看所有日程", command=self.show_all_schedule
        )
        ...
        self.edit_schedule_button = tk.Button(
            self.button_frame,
            text="编辑日程",
            font=("Arial", 18),
            width=10,
        ...
        self.delete_schedule_button = tk.Button(
            self.button_frame,
            text="删除日程",
            font=("Arial", 18),
            width=10,
    ...
    def create_calendar(self):
        ...
        for i in range(days_in_month):
            day_button = tk.Button(
                self.calendar_frame, text=str(i + 1), width=2, font=("Arial", 20)
            )
    ...
    def schedule_display(self, event):
        ...
        with open("date", "r", encoding="utf-8") as f:
            ...
            for i in range(len(lines)):
                ...
                if (
                    start_date_list[0] > int(year)
                    or (
                        start_date_list[0] == int(year)
                        and start_date_list[1] > int(month)
                    )
                    or (
                        start_date_list[0] == int(year)
                        and start_date_list[1] == int(month)
                        and start_date_list[2] > int(DayInMonth)
                    )
                ):
                    continue
                elif (
                    end_date_list[0] < int(year)
                    or (end_date_list[0] == int(year) and end_date_list[1] < int(month))
                    or (
                        end_date_list[0] == int(year)
                        and end_date_list[1] == int(month)
                        and end_date_list[2] < int(DayInMonth)
                    )
                ):
                    continue
                else:
    ...
    def delete_schedule(self):
        ...
        # 创建一个按钮，点击后删除这个日程
        del_schedule_button = tk.Button(
            edit_schedule_window,
            text="确定",
            command=lambda: delete_schedule_confirm(self, LineInList),
        )
        del_schedule_button.pack(side=tk.LEFT, padx=10)
        # 创建一个按钮，点击后关闭这个窗口
        close_schedule_button = tk.Button(
            edit_schedule_window, text="取消", command=edit_schedule_window.destroy
        )
    ...
    def edit_schedule(self):
        ...
        add_button = tk.Button(
            edit_schedule_window,
            text="确认修改",
            command=lambda: self.edit_schedule_to_file(
                date_start_entry.get_date(),
                time_start_entry.get(),
                date_end_entry.get_date(),
                time_end_entry.get(),
                type_entry.get(),
                content_entry.get(),
                warning_label,
    ...
    def edit_schedule_to_file(
        ...
        try:
            ...
            if len(time_start_list) != 2 or len(time_end_list) != 2:
                ...
                return
            if (
                time_start_list[0] > 23
                or time_start_list[1] > 59
                or time_end_list[0] > 23
                or time_end_list[1] > 59
            ):
                print("时间格式错误")
                warning_label.config(text="时间格式错误")
                return
        if date_start > date_end:
            ...
            return
        if date_start == date_end:
            if time_start_list[0] > time_end_list[0] or (
                time_start_list[0] == time_end_list[0]
                and time_start_list[1] > time_end_list[1]
            ):
                print("开始时间不能大于结束时间")
    ...
    def add_schedule(self):
        ...
        add_button = tk.Button(
            add_schedule_window,
            text="添加",
            command=lambda: self.add_schedule_to_file(
                date_start_entry.get_date(),
                time_start_entry.get(),
                date_end_entry.get_date(),
                time_end_entry.get(),
